...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        config_args = {
            "gpt2_topo": dict(n_layer=12, n_head=16, n_embd=784),
    ...
    @torch.no_grad()
    def generate(self, enc, prompt, max_new_tokens, top_k=50):
        tokens = enc.encode(prompt)
        tokens = torch.tensor(tokens, dtype=torch.long)
        xgen = tokens.unsqueeze(0).to(device)
        ...
        tokens = xgen[0, :max_new_tokens].tolist()
